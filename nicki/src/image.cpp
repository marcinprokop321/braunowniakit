...
bool Image_toAscii(struct image* img, int sizex, int sizey,int level, bool negative)
{
	if (img == nullptr)
	{
		printf("WRONG INDEX!\n");
	...
	for(int y = 0; y < sizey; y++)
	{
		...
		{
			uint32_t chartoPrint = 10240+(negative*255);
			for (int i = 0; i < 8; i++)
			{
				int pixelSum = 0;
				...
				if (pixelSum <= level)
					chartoPrint += aha2[i]* -(negative);
			}
			if (chartoPrint == 10240+(255*negative))
				chartoPrint += 4-(negative * 255);
...
void Image_Pars(int i, std::vector<std::string> args)
{
	...
	case 3:
		...
		Image_toAscii(&images[imageId - 1], scalex, scaley, level,atoi(args[6].c_str()));
		break;
	case 4:
		if (args.size() < 5)
		{
